...
import { getExplorerClickAction } from "./explorerNodeBehavior";
import {
...
export class RojoExplorerProvider implements vscode.TreeDataProvider<ExplorerNode> {
  ...
  canEditResourceMeta(node?: ExplorerNode): boolean {
    const source = node?.instance?.source;
    if (node?.kind !== "instance" || !node.resourceUri || !source?.exists || !source.entryType) {
      return false;
    }
